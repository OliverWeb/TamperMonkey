;(function ($, window, document, undefined) {
    ...
    Drag.prototype = {
        run: function () {
            ...
            if (parent == 'parent') {parent = element.parent()} else {
                parent = element.parents(parent)
            }
            if (!handler) {handler = element} else {
            ...
            function randomPlace () {
                if (randomPosition) {
                    ...
                    var randY = parseInt(
                      Math.random() * (boxHeight - sonHeight))
                    if (direction.toLowerCase() == 'x') {
                        element.css({left: randX})
                    } else if (direction.toLowerCase() == 'y') {
                        element.css({top: randY})
                    } else {element.css({left: randX, top: randY})}
                }
            }
            ...
            handler.css({cursor: 'move'}).mousedown(function (e) {
                ...
                element.addClass('on')
                fun.dragStart(parseInt(element.css('left')),
                  parseInt(element.css('top')))
                return false
            })
            $(document).
              mouseup(function (e) {
                  fun.dragEnd(parseInt(element.css('left')),
                    parseInt(element.css('top')))
            ...
            $(document).mousemove(function (e) {
                ...
                if (isDown) {
                    fun.dragMove(parseInt(element.css('left')),
                      parseInt(element.css('top')))
                } else {return false}
                if (direction.toLowerCase() ==
                  'x') {thisXMove()} else if (direction.toLowerCase() ==
                  'y') {thisYMove()} else {thisAllMove()}
            })
        }
    }
